  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-zinc-50/50 text-zinc-500 text-xs uppercase tracking-[0.15em] font-black border-b border-zinc-100">
            {headers.map((header, i) => (
              <th key={i} className={`px-6 py-3 font-black ${i === headers.length - 1 ? 'text-right' : ''}`}>
                {header}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-zinc-100">
          {loading ? (
            ...
            <tr>
              <td colSpan={headers.length} className="py-8 text-center text-zinc-400 text-xs italic">
                {emptyMessage}
              </td>
            </tr>
          ) : (
